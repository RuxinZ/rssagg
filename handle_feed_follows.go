...
func (apiCfg *apiConfig) handlerCreateFeedFollow(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, fmt.Sprintf("Error decode params: %v", err))
		return
	}

	feedFollows, err := apiCfg.DB.GetFeedFollowsForUser(r.Context(),user.ID)
	if err != nil  {
		respondWithError(w, http.StatusInternalServerError, fmt.Sprintf("Couldn't verify user's following feeds: %v", err))
		return
	}

	for _, feedFollow := range feedFollows {
		if feedFollow.FeedID == params.FeedID {
			respondWithError(w, http.StatusBadRequest, fmt.Sprintln("User is already following this feed"))
		return
		}
	}
